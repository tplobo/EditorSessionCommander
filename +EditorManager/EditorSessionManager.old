...
% Editied by SteveD so that the functions could be called by startup.m and
% finish.m to automate session management.
% (https://www.mathworks.com/matlabcentral/answers/344124-restore-open-files-in-editor-from-previous-sessions)
%
% Add to startup.m: editorLayout.SessionEditor.openSession(pwd,'c');
% Add to finish.m:  editorLayout.SessionEditor.saveSession(pwd,false);

classdef EditorManager
        ...
        function openSession(NameIn,TreatmentOfOpenFiles)
            ...
            if nargin < 1
                ...
                for iname = 1:size(T,1)
                    if strcmp(T.name{iname},NameIn), indexFound = T.index(iname); end
                end
                if isempty(indexFound)
                    disp('Session name not recognized.');
                    indexFound = chooseOption(T);
                end
            end
            ...
            if any(editorFileStatus==0)
                ...
                if exist('TreatmentOfOpenFiles','var') && ~isempty(TreatmentOfOpenFiles)
                    universalResponse = TreatmentOfOpenFiles;
                else
